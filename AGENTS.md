...
## Railway first deploy — September 23, 2026
- Deployed to Railway (project `redactme`, one service, GitHub `jgotti1/RedactMe` `main`, default domain `redactme-production.up.railway.app`; custom domain `redactme.margotticode.com` attached, Bluehost CNAME pending). Full steps, variables, DNS and gotchas are in [DEPLOYMENT.md](DEPLOYMENT.md) (secret-free).
- First-deploy finding: a redeploy overlaps old and new containers for about 10 seconds and discards in-memory documents, so an in-flight upload returned 404 on `/redact` afterwards. Not a code bug; re-upload resolves it. The redact error is shown in the review banner/note but is easy to miss; consider making a 404 "upload expired" message more prominent with a re-upload prompt.
- Custom domain status (September 23, 2026): Bluehost CNAME `redactme` -> Railway target added and propagated; Railway certificate still "validating ownership" (custom domain shows Railway's "Not Found" page until issued). `redactme.margotticode.com` added to Firebase authorized domains. Next: confirm the certificate issues (retry via Railway if stuck after 30-60 minutes), then test `/api/health` and sign-in on the custom domain.
- Custom domain live (September 23, 2026): the certificate was stuck because the Bluehost `_railway-verify.redactme` TXT record was missing (CNAME alone is not enough). After adding it, `https://redactme.margotticode.com/api/health` returns 200 with a valid certificate. Sign-in initially failed because the Firebase authorized domain was misspelled (`marggotticode`); corrected and sign-in works. Railway plan is Hobby (usage-based); keep one replica/worker.
