// This page is a UI preview. It never reads, uploads, or produces PDF files.
  ...
  root.innerHTML = `
    ...
    <div class="workspace-grid">
      <section class="document-area" aria-labelledby="document-title"><div class="panel-heading"><div><p class="eyebrow">YOUR DOCUMENT</p><h2 id="document-title">Start with a PDF</h2></div><span id="document-tag" class="workspace-label">No document</span></div>
        <div id="upload-empty" class="upload-empty"><span class="upload-symbol" aria-hidden="true">↑</span><h3>Your next document starts here</h3><p>Upload a PDF to find sensitive details<br>and review suggested redactions.</p><button class="primary" type="button" disabled aria-describedby="upload-note">Choose PDF <span aria-hidden="true">+</span></button><p id="upload-note" class="availability">PDF upload is coming next.</p><button id="sample-open" class="text-button" type="button">Explore a sample review →</button></div>
